...
async def create_disease(disease_name: str, description: str = None, db_session: AsyncSession = Depends(get_db)):
...
async def get_diseases(db_session: AsyncSession = Depends(get_db)):
    diseases = await list_diseases(db_session)
    return {"diseases": diseases}

@router.post("/update/{disease_id}", status_code=200)
async def modify_disease(disease_id: int, disease_name: str = None, description: str = None, db_session: AsyncSession = Depends(get_db)):
...
async def remove_disease(disease_id: int, db_session: AsyncSession = Depends(get_db)):
